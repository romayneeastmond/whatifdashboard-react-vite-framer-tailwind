# What If Dashboard Ideas

## Existing Tools

- ~~Salary & Taxes~~ ✓
- ~~Mortgage Equity~~ ✓
- ~~Wealth Growth~~ ✓
- ~~Goals Tracking~~ ✓
- ~~Time Allocation~~ ✓
- ~~Bardal Factor~~ ✓
- ~~Debt Repayment~~ ✓
- ~~Days Between~~ ✓
- ~~Weight Loss~~ ✓
- ~~Protein Intake~~ ✓
- ~~Calorie Deficit Planner~~ ✓
- ~~Career Path Projection~~ ✓
- ~~Wrongful Dismissal~~ ✓

---

# Financial & Career Modeling

- Net Worth Projection
- FIRE / Retirement Calculator
- RRSP vs TFSA Optimizer
- Rent vs Buy Calculator
- Emergency Fund Runway
- Job Offer Comparator
- Severance & EI Estimator
- Side Hustle Profitability
- Inflation Impact Calculator
- Cost of Delay Calculator

---

# Productivity & Lifestyle

- Habit Consistency Tracker
- Burnout Risk Indicator
- Commute Time Cost
- Meeting Cost Calculator
- Digital Time Audit
- Opportunity Cost Simulator

---

# Health & Fitness

- ~~Calorie Deficit Planner~~ ✓
- BMI + Body Fat Estimator
- Running Pace Calculator
- Macro Nutrient Planner
- Sleep Debt Tracker
- VO2 Max Estimator
- Water Intake Calculator
- Habit-to-Weight Projection

---

# Family & Life Planning

- Child Education Savings Projection
- Life Milestone Timeline
- Cost of Raising a Child Estimator
- Relationship Finance Split Tool

---

# Advanced “What If” Simulators

- Layoff Survival Simulator
- What If I Move? Calculator
- What If I Take a Lower Paying Job?
- ~~Career Path Projection~~ ✓
- Lifestyle Inflation Simulator
- AI Replacement Risk Estimator

---

# Legal & Family Law Calculators

## Family Law

- Child Support Estimator
- Spousal Support / Alimony Estimator
- Shared Custody Expense Split
- Separation Asset Split Calculator
- Parenting Time Calculator
- Divorce Cost Estimator

## Estate & End-of-Life Planning

- Estate Value Estimator
- Probate Fee Calculator
- Inheritance Split Calculator
- Life Insurance Needs Calculator
- Funeral Cost Planner
- Executor Compensation Estimator

## Employment & Workplace

- Severance Estimator
- ~~Wrongful Dismissal Estimate~~ ✓
- Overtime Pay Calculator
- Contractor vs Employee Income Comparison
- Vacation Pay Calculator
- Employment Insurance (EI) Estimator
- Maternity / Parental Leave Benefit Calculator

## Debt & Consumer Law

- Bankruptcy vs Consumer Proposal Comparison
- Wage Garnishment Estimator
- Interest Penalty Calculator
- Small Claims Court Limit Checker
- Loan Default Recovery Timeline

## Housing & Real Estate Legal Tools

- Land Transfer Tax Calculator
- Closing Cost Estimator
- Tenant Rent Increase Calculator
- Mortgage Penalty Calculator
- Capital Gains on Property Sale

## Immigration & International

- Express Entry CRS Score Calculator
- Visa Financial Requirement Estimator
- Cross-Border Tax Comparison
- Relocation Cost Estimator

---

# Unique Platform Features

- Scenario Save & Compare
- Timeline Playback
- Sensitivity Analysis
- AI Recommendations
- Unified Life Score

---

# Differentiation & Export Ideas

## Export to Known Formats
- Mortgage/Debt → amortization table export (Excel/CSV)
- Weight Loss Plan → printable PDF or MyFitnessPal-compatible format
- Days Between / Time Allocation → Google Calendar / iCal (.ics) export
- Offer exporting to Notion or Obsidian

## ~~Cross-Calculator Scenario Comparison~~ (tried, rejected)
- NOTE: Per-calculator multi-person comparison already covers this. A global scenario just duplicates that without adding immediate visual feedback. Removed.
- NOTE: Debt and Mortgage calculators already support per-calculator scenario comparison (multiple people/scenarios within one calculator)
- This idea is different: a *global* named life scenario that spans multiple calculators at once
- e.g. "Scenario A: take lower-paying job" saves inputs across Salary, Debt, Wealth Growth, and Goals simultaneously
- Lets users compare full life outcomes, not just single-calculator outputs
- Reinforces the "what-if" brand and is hard for generic calculator sites to replicate quickly

---

# Suggested Product Categories

- Finance
- Career
- Health
- Family
- Legal
- Retirement
- Productivity
- Life Planning